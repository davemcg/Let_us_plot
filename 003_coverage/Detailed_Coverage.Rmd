...
x <- dd_processed %>% filter(Transcript %in% tx) %>% group_by(Transcript, `Exon Number`) %>% 
    expand(Start=full_seq(c(Start,End),1)) %>% 
...
y <- x %>% group_by(Name) %>% mutate(Pos = 1:n())
z <- y %>% group_by(Name, `Exon Number`) %>% summarise(Start=min(Pos), End=max(Pos)) %>% mutate(Exon = case_when(as.numeric(`Exon Number`) %% 2 == 0 ~ 'even', TRUE ~ 'odd'))
zz<-bind_rows(y,z)
