...
```{r}
cobra_eel_AUC <- rbind(cobra %>% filter(Output == 'Area-Under-Curve'), eel %>% filter(Output == 'Area-Under-Curve'))
cobra_eel_AUC %>% 
  ggplot(aes(x=Week, y=Data, colour=Region)) + 
  geom_point() 
```
...
# N1P1 Plot
```{r}
rbind(cobra %>% filter(Output == 'N1P1'), eel %>% filter(Output == 'Area-Under-Curve')) %>% 
  ggplot(aes(x=Week, y=Data, colour=Region, shape = Pig_Name)) + 
  geom_point(size=4) + 
  geom_smooth() + # this draws the smoothed lines through the four points. It auto picks an algorithm that works. loess was used here
  theme_bw() + scale_colour_npg() +
...
# Latency plot
```{r}
rbind(cobra %>% filter(Output == 'N1-Latency'), eel %>% filter(Output == 'Area-Under-Curve')) %>% 
  ggplot(aes(x=Week, y=Data, colour=Region, shape = Pig_Name)) + 
  geom_point(size=4) + 
  geom_smooth() + # this draws the smoothed lines through the four points. It auto picks an algorithm that works. loess was used here
  theme_bw() + scale_colour_npg() +
...
ggplot(all_pigs, aes(x=PC1, y=PC2, color=as.factor(Week), shape=Region)) + 
  geom_point(size=5) + 
  theme_bw() + 
  scale_colour_npg() 
```
