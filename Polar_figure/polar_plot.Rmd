...
```{r}
polar_values <- polar_values %>% slice(1:5) %>% mutate(`Od Amp1` = as.numeric(as.character(`Od Amp1`)), Patient=X__1) %>% select(Patient,`Od Amp1`:`Od Amp3`, `Od Angle1`:`Od Angle3`)
polar_values
```
...
We drop the now useless Test_Session column and then us the `spread` function to use the Test and Value columns to get it wide. 
```{r}
polar_values %>% 
  gather(Test_Session, Value, `Od Amp1`:`Od Angle3`) %>% 
  mutate(Session = str_sub(Test_Session, -1), Test = str_sub(Test_Session,1,-2)) %>% 
